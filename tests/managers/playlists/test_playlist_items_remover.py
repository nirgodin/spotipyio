...
class TestPlaylistsItemsRemover:
    async def test_run__valid_request__returns_last_removal_snapshot_id(
        self,
        ...
        test_client.playlists.remove_items.expect_success(
            playlist_id=playlist_id, uris=uris, snapshot_id=first_snapshot_id, expected_snapshots=expected_snapshots
        )

        actual = await spotify_client.playlists.remove_items.run(
            playlist_id=playlist_id, uris=uris, snapshot_id=first_snapshot_id
        )
